...
ZO_CreateStringId("LOOTSANITIZER_WARNING"                     , "|cff0000Внимание!|r |cc5c29eУдалённые предметы нельзя вернуть.|r")
...
ZO_CreateStringId("LOOTSANITIZER_EQUIPMENT_CONTROL"           , "Удаление обычной экипировки")
...
ZO_CreateStringId("LOOTSANITIZER_EQUIPMENT_DESCRIPTION"       , "Простая экипировка без ремесленных особеностей и стоимостью <<1>>.") -- 1:price
ZO_CreateStringId("LOOTSANITIZER_SIMPLECLOTHES_DESCRIPTION"   , "Одежда – экипировка без статов и бонусов набора. Обычно используется для ролеплея и стоит около <<1>>. Пример: <<2>>") -- 1:price 2:itemLink example
...
ZO_CreateStringId("LOOTSANITIZER_TRASH_DESCRIPTION"           , "Удаление предметов из категории «Мусор», которые стоят 1 золотую. К примеру, <<1>>.")
...
ZO_CreateStringId("LOOTSANITIZER_JUNK_COMMON_CONTROL"         , "Обычная экипировка")
...
ZO_CreateStringId("LOOTSANITIZER_JUNK_AUTOSELL_DESCRIPTION"   , "Автоматическая продажа предметов из вкладки «Хлам» торговцу.")
